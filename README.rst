########################################
Building and uploading enaml wheels
########################################
...
https://travis-ci.org/MacPython/enaml-wheels

Appveyor interface at
https://ci.appveyor.com/project/matthew-brett/enaml-wheels
...
* builds a enaml wheel, linking against these fresh builds;
...
* making a commit to the `enaml-wheels` repository (e.g. with `git
...
Which enaml commit does the repository build?
===============================================

The `enaml-wheels` repository will build the commit specified in the
...
Note: when making a enaml release, it's best to only push the commit (not the
tag) of the release to the ``enaml`` repo, then change ``BUILD_COMMIT`` to the
...
be something like::
    ...
    wheel-uploader -u $CDN_URL -s -v -w ~/wheelhouse -t all enaml $VERSION
...
``enaml`` is the root name of the wheel(s) to download / upload, and
